...
def swap_nan_with_zero(dict):
    return {k: 0 if np.isnan(v) else v for k, v in dict.items()}
...
def get_json_from_dict(merged_table):
    to_return = list()
...
if __name__ == '__main__':
    merged_table, genres_cols = merge_data_frames(4)
    print(merged_table)
    rows = get_json_rows(merged_table)
    print(get_single_user_ratings(merged_table,genres_cols,75))